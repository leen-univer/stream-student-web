...
export const getDiscountValue = (MRP: number, salePrice: number) =>
  (((MRP - salePrice) / MRP) * 100).toFixed(2) || 0;
...
// export const currencyFormatterDollar = (value: number) => {
//   return new Intl.NumberFormat("en-US", {
//     style: "currency",
//     currency: "USD",
//   }).format(value);
...
//   const authToken = getLocalStorageItem('RefreshToken')
...
export const post: APIFunction = async ({
  ...
  try {
    ...
    if (response?.status === 401) {
      ...
      headers.Authorization = `$Bearer ${getResponseData?.token}`;
      const response = await fetch(`${BASE_URL}${path}`, API_OPTIONS);
