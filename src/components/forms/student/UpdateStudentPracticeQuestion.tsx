...
export default function UpdateStudentPracticeQuestion({
  data,
  mutate,
}: {
  data: any;
  ...
  const formik = useFormik({
    initialValues: data
      ? {
          ques: data?.question,
          answer: data?.answerData[0]?.answer,
        }
      : initialValues,
    ...
    onSubmit: async (values) => {
      try {
        ...
        res = await mutation(
          `longAnswer/update-long-answer/${data?.answerData[0]?._id}`,
          {
            method: "PUT",
            body: formData,
            isFormData: true,
