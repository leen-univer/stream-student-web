...
export default function AddQuestionDialog({ mutate }: { mutate: () => void }) {
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (router?.query?.videoId) {
          res = await mutation(
            `tutor/create-question?sectionId=${router?.query?.videoId}&assignmentId=${router?.query?.videoId}`,
            {
              method: "POST",
              body: formData,
              isFormData: true,
  ...
  return (
    <React.Fragment>
      <div
        ...
        className="w-[80%] m-auto cursor-pointer justify-center flex items-center gap-1 border border-primary bg-primary/10 text-primary px-3 py-1 rounded-md"
      >
