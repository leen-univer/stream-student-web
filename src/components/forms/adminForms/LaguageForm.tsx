...
import { CategoryFormContent, LanguageFormContent } from "locale";
  ...
  const inputSchema = [
    {
      key: "1",
      name: "languageName",
      label: `${LanguageFormContent(selectedLanguage).LanguageName} `,
      required: true,
      placeholder: `${LanguageFormContent(selectedLanguage).EnterLanguageName}`,
      initialValue: "",
      validationSchema: yup
        .string()
        .matches(
          /^[a-zA-Z]+$/,
          LanguageFormContent(selectedLanguage).LanguageNameOnlyCharacters
        )
        .required(LanguageFormContent(selectedLanguage).LanguageNameIsRequired),
      type: "text",
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        formData?.append("options", values?.languageName);

        res = await mutation("language/create-language", {
          method: "POST",
          body: formData,
          isFormData: true,
          isAlert: true,
          ...
          >
            {loading ? (
              <>
                ...
                />
                {LanguageFormContent(selectedLanguage).Adding}
              </>
            ) : (
              LanguageFormContent(selectedLanguage).AddLanguage
            )}
          </button>
        </div>
      </div>
