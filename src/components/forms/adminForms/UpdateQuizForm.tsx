...
import { IQuestionCard } from "types/questionCard";
...
export default function UpdateQuizForm({
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        res = await mutation(`quizQuestion/update-quizQuestion/${data?._id}`, {
          method: "PUT",
          body: formData,
          isFormData: true,
          isAlert: true,
  ...
  return (
    <React.Fragment>
      <Tooltip
        title={lectureSectionPage(selectedLanguage).updateQuestion}
        followCursor
        placement="top-start"
        arrow
      >
