...
import { dataContent, deleteContent } from "locale";

const IndividualAssignment = () => {
  const { selectedLanguage } = useAppContext();
  const router = useRouter();
  const { data, mutate } = useSWRAPI(
    `tutor/all-assignment-question?sectionId=${router?.query?.assignmentId}`
  );
  return (
    <TutorPanelLayout title="View Assignment | StreamStudent">
      <div className="main-container bg-white rounded-lg p-6">
      ...
      {data?.length === 0 ? (
        <div>
          <h1 className="text-3xl font-bold text-primary text-center pb-2">
            {dataContent(selectedLanguage).Assignment}
          </h1>
      ...
      <div className=" flex flex-row-reverse justify-between gap-7 text-xl font-medium">
        <div className="whitespace-nowrap text-right">
          <p className="font-semibold text-lg text-primary">
            {dataContent(selectedLanguage).Marks}: {data?.section?.marks}
          </p>
          <p>
            {" "}
            {dataContent(selectedLanguage).Answer}: {data?.section?.answer}
          </p>
        </div>
